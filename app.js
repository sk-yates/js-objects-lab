...
game.party.push(pokemon[91], pokemon[119], pokemon[132]);
...
//function ".gymBattles" added to "const game".
...
game.catchPokemon = function(pokemonObj) {
  if (game.party.length < 6) {  // if party number is 6 run the following code
    game.party.push(pokemonObj);
    game.items[1].quantity = game.items[1].quantity - 1; 
  } else {
...
catchPokemon(pokemon[50]);

console.log("-------------------- Party Limit: 6 --------------------")
...
game.catchPokemon = function(pokemonName) {
  const pokemonFound = pokemon.find(pokemon => {
...
// pokemon.forEach(pokemon => { 
//   if (!loggedTypes.has(pokemon.type)) {
//     console.log(pokemon.type);
//     loggedTypes.add(pokemon.type);
// }});
